...
async function loadOptionsFromBackend() {
  try {
    const resp = await fetch('http://localhost:8080/api/opcoes');
    if (resp.ok) {
...
function setupModalEvents() {
  ...
  document.getElementById('confirmMethod').addEventListener('click', async () => {
    ...
    try {
      ...
      const resp = await fetch(`http://localhost:8080/api/despesas/${currentDespesaId}/pago?valor=true`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  document.getElementById('confirmDelete').addEventListener('click', async () => {
    ...
    try {
      const resp = await fetch(`http://localhost:8080/api/despesas/${currentDespesaId}`, {
        method: 'DELETE'
      });
...
function render() {
  ...
  const qs  = buildQuery();
  const url = `http://localhost:8080/api/despesas${qs ? '?' + qs : ''}`;
      ...
      for (const d of list) {
        ...
        tr.querySelector('input').addEventListener('change', e => {
          if (e.target.checked) {
            ...
            fetch(
              `http://localhost:8080/api/despesas/${d.id}/pago?valor=false`,
              { 
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({})
